...
const tools: Tool[] = [
  { title: 'Merge PDF', desc: 'Combine multiple PDF files into one document.', icon: Layers, implemented: true, path: '/merge', category: 'Edit', color: 'text-cyan-500', bg: 'bg-cyan-50 dark:bg-cyan-950/20' },
...
function QuickDropModal({ file, onClear, onBack }: { file: File, onClear: () => void, onBack?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="w-full max-w-md bg-[#F5FBFF] dark:bg-zinc-950 rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden border-t border-x border-white/10 sm:border animate-in slide-in-from-bottom-full duration-500 ease-out">
        ...
        <div className="p-6 pb-2">
          <div className="flex items-center justify-between mb-6">
             <div className="flex items-center gap-3">
                {onBack && (
                  <button onClick={onBack} className="p-2 -ml-2 text-gray-400 hover:text-cyan-500 transition-colors">
                    <ChevronDown className="rotate-90" size={20} />
                  </button>
                )}
                <div className="w-10 h-10 bg-cyan-500 text-white rounded-xl flex items-center justify-center shadow-lg shadow-cyan-500/20">
                   <FileText size={20} />
                </div>
             ...
             <button onClick={onClear} className="p-2 bg-gray-100 dark:bg-zinc-900 rounded-full text-gray-400 hover:text-cyan-500 transition-colors"><X size={18}/></button>
          </div>
        </div>
        ...
        <div className="px-6 pb-6 space-y-4 max-h-[60vh] overflow-y-auto scrollbar-hide">
           ...
           <div>
              <button 
                ...
                className="w-full flex items-center justify-between p-4 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-white/5 rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-cyan-500 transition-colors shadow-sm"
              >
...
function App() {
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    ...
    if (typeof window !== 'undefined') {
      const mobileLike = window.innerWidth < 768 || /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
      return mobileLike ? 'android' : 'web'
    }
    return 'web'
  })
  ...
  const LoadingSpinner = () => (
    <div className="h-full w-full flex items-center justify-center bg-[#F5FBFF] dark:bg-[#030712] min-h-[60vh]">
            ...
            {import.meta.env.DEV && (
              <div className="fixed bottom-24 right-6 z-[100] flex flex-col gap-2">
                <button
                  ...
                  className="bg-gray-900 dark:bg-zinc-800 text-white p-4 rounded-3xl shadow-2xl hover:bg-cyan-500 transition-all duration-300 flex items-center gap-3 border border-white/10 group active:scale-95"
