...
const ToggleSwitch = ({ checked, onChange }: { checked: boolean, onChange: () => void }) => (
  <button 
    ...
    className={`w-12 h-7 rounded-full p-1 transition-all duration-300 ${checked ? 'bg-cyan-500 shadow-lg shadow-cyan-500/20' : 'bg-gray-200 dark:bg-zinc-700'}`}
  >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 flex-1 overflow-hidden">
        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 transition-colors ${danger ? 'bg-red-50 dark:bg-red-900/20 text-red-500' : (iconColor || 'bg-gray-100 dark:bg-zinc-800 text-gray-500 group-hover:text-cyan-500 group-hover:bg-cyan-50 dark:group-hover:bg-cyan-950/20')}`}>
    ...
    <NativeToolLayout title="System" description="Core Configuration" actions={null}>
      <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 pb-40">
        ...
        <div className="flex items-center gap-4 px-2 mb-8 mt-2">
           <div className="w-12 h-12 bg-cyan-500 rounded-2xl flex items-center justify-center shadow-lg shadow-cyan-500/20 text-white shrink-0">
          ...
          <div className="p-2 grid grid-cols-3 gap-2">
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center gap-2 py-3.5 rounded-[1.25rem] transition-all border border-transparent ${theme === t.id ? 'bg-zinc-950 dark:bg-white text-white dark:text-black shadow-xl scale-[1.02]' : 'bg-gray-50 dark:bg-[#030712]/40 text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-800'}`}
              >
        ...
        <SettingGroup title="Privacy">
          ...
          />
          {autoWipe && (
            <div className="px-5 py-3 flex items-center justify-between bg-cyan-50/50 dark:bg-cyan-950/10 border-t border-cyan-100/20 dark:border-cyan-950/20 animate-in slide-in-from-top-2">
               <span className="text-[9px] font-black text-cyan-500 uppercase tracking-widest">Wipe Delay</span>
               <select 
                value={wipeTimer}
                onChange={(e) => handleSelect('autoWipeTimer', e.target.value, setWipeTimer)}
                className="bg-transparent text-[11px] font-black text-cyan-500 outline-none cursor-pointer text-right"
               >
          ...
          <div className="px-5 py-4 flex items-center justify-between">
             ...
             <select 
              ...
              className="bg-gray-100 dark:bg-[#030712] px-3 py-2 rounded-xl text-[11px] font-black text-gray-600 dark:text-gray-300 outline-none border border-transparent focus:border-cyan-500 cursor-pointer"
             >
