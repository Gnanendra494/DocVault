...
export default function AndroidHistoryView() {
  ...
  return (
    <div className="min-h-screen bg-[#F5FBFF] dark:bg-[#030712] pb-32 transition-colors">
      <header className="px-6 pt-[calc(env(safe-area-inset-top)+1rem)] pb-6 sticky top-0 bg-[#F5FBFF]/90 dark:bg-[#030712]/90 backdrop-blur-xl z-50 border-b border-gray-100 dark:border-white/5">
        <div className="flex items-center justify-between mb-6">
          <div className="flex flex-col text-left">
            <h1 className="text-3xl font-black tracking-tighter dark:text-white">Activity</h1>
            <p className="text-[10px] font-black uppercase tracking-widest text-cyan-500 opacity-80">Local Storage Active</p>
          </div>
          {history.length > 0 && (
            <button 
              onClick={handleClear}
              className="p-3 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-2xl active:scale-90 transition-all shadow-sm"
            >
              <Trash2Icon size={20} />
            </button>
          )}
        ...
        {filteredHistory.length === 0 ? (
          ...
          filteredHistory.map((item) => (
            <div key={item.id} className="p-4 bg-white dark:bg-zinc-900 rounded-[2rem] border border-gray-100 dark:border-white/5 flex items-center gap-4 active:scale-[0.99] transition-all shadow-sm group">
              <div className="w-12 h-12 bg-gray-50 dark:bg-zinc-800 text-gray-400 group-hover:bg-cyan-50 dark:group-hover:bg-cyan-950/20 group-hover:text-cyan-500 rounded-2xl flex items-center justify-center shrink-0 transition-colors shadow-inner">
              ...
              <div className="flex items-center gap-2">
                 {item.resultUrl && (
                    <a 
                      href={item.resultUrl} 
                      download={item.name} 
                      className="w-10 h-10 bg-cyan-500 text-white rounded-full flex items-center justify-center shadow-lg shadow-cyan-500/20 active:scale-90 transition-all"
                    >
