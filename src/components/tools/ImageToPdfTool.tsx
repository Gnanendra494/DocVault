...
function SortableImageItem({ id, img, onRemove }: { id: string, img: ImageFile, onRemove: (id: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center gap-3 p-3 bg-white dark:bg-zinc-900 rounded-2xl border transition-colors shadow-sm group touch-none relative ${isDragging ? 'border-cyan-500 shadow-xl scale-[1.02]' : 'border-gray-100 dark:border-white/5'}`}>
      <div {...attributes} {...listeners} className="p-2 cursor-grab text-cyan-300 hover:text-cyan-500 active:scale-90 transition-transform"><GripVertical size={20} /></div>
      <div className="w-12 h-16 bg-gray-100 dark:bg-[#030712] rounded-lg overflow-hidden shrink-0 border border-gray-200 dark:border-zinc-800"><img src={img.preview} alt="P" className="w-full h-full object-cover" /></div>
      <div className="flex-1 min-w-0"><p className="font-bold text-sm truncate dark:text-white">{img.file.name}</p></div>
      <button onClick={() => onRemove(id)} className="p-2 text-gray-400 hover:text-cyan-500"><X size={18} /></button>
    </div>
  )
}
...
export default function ImageToPdfTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-images-to-pdf')
  ...
  const ActionButton = () => (
    <button onClick={convertToPDF} disabled={isProcessing || images.length === 0} className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 shadow-xl shadow-cyan-500/20 ${isNative ? 'py-4 rounded-2xl text-sm' : 'p-6 rounded-3xl text-xl'}`}>
  ...
  return (
    <NativeToolLayout title="Image to PDF" description="Convert photos and images into a professional PDF." actions={images.length > 0 && !downloadUrl && <ActionButton />}>
      ...
      {images.length === 0 ? (
        <div onClick={() => fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 dark:hover:bg-cyan-950/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><Upload size={32} /></div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center px-1"><p className="text-[10px] font-black uppercase text-gray-400">{images.length} Images</p><button onClick={() => setImages([])} className="text-[10px] font-black uppercase text-cyan-500/60">Clear</button></div>
          ...
          <button onClick={() => fileInputRef.current?.click()} className="w-full py-3 border-2 border-dashed border-gray-200 dark:border-zinc-800 rounded-2xl text-gray-400 font-bold text-sm flex items-center justify-center gap-2 hover:border-cyan-500 hover:text-cyan-500 transition-all"><Plus size={16} /> Add More</button>
          <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-2">Filename</label><input type="text" value={customFileName} onChange={(e) => setCustomFileName(e.target.value)} className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 border border-transparent focus:border-cyan-500 outline-none font-bold text-sm" /></div>
