...
export default function WatermarkTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-watermarked')
  ...
  const ActionButton = () => (
    <button onClick={applyWatermark} disabled={isProcessing || !text} className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl flex items-center justify-center gap-3 shadow-lg shadow-cyan-500/20`}>
    ...
    <NativeToolLayout title="Watermark" description="Add secure text overlays to your documents locally." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 dark:hover:bg-cyan-950/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><Type size={32} /></div>
        ...
        <div className="max-w-md mx-auto relative z-[100]">
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 text-center shadow-2xl">
            ...
            <input type="password" value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} placeholder="Password" className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-4 border border-transparent focus:border-cyan-500 outline-none font-bold text-center mb-4 dark:text-white" />
              ...
              {!downloadUrl ? (
                <>
                  <div>
                    ...
                    <input type="text" value={text} onChange={(e) => setText(e.target.value)} className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 border border-transparent focus:border-cyan-500 outline-none font-bold text-sm dark:text-white" />
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="col-span-2">
                          ...
                          <div className="flex gap-2 flex-wrap">
                             {['#00F2FE', '#3B82F6', '#10B981', '#F59E0B', '#000000'].map(c => (
                               <button 
                                 key={c} 
                                 onClick={() => setColor(c)}
                       ...
                       <div>
                          ...
                          <input type="range" min="0.1" max="1" step="0.1" value={opacity} onChange={(e) => setOpacity(parseFloat(e.target.value))} className="w-full accent-cyan-500" />
                       </div>
                       <div>
                          <label className="block text-[8px] font-black uppercase text-gray-400 mb-2">Size ({fontSize}px)</label>
                          <input type="range" min="10" max="200" step="1" value={fontSize} onChange={(e) => setFontSize(parseInt(e.target.value))} className="w-full accent-cyan-500" />
                       </div>
                       <div>
                          <label className="block text-[8px] font-black uppercase text-gray-400 mb-2">Rotation ({rotation}°)</label>
                          <input type="range" min="-180" max="180" step="5" value={rotation} onChange={(e) => setRotation(parseInt(e.target.value))} className="w-full accent-cyan-500" />
                       </div>
                    </div>
                  </div>
