...
export default function PdfToTextTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-extracted')
  ...
  const ActionButton = () => (
    <button onClick={handleStartExtraction} disabled={isProcessing} className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 shadow-xl shadow-cyan-500/20 ${isNative ? 'py-4 rounded-2xl text-sm' : 'p-6 rounded-3xl text-xl'}`}>
  ...
  return (
    <NativeToolLayout title="PDF to Text" description="Extract text using fast scan or deep local OCR." actions={pdfData && !pdfData.isLocked && !extractedText && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 dark:hover:bg-cyan-950/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><FileText size={32} /></div>
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 text-center">
            <div className="w-16 h-16 bg-cyan-100 dark:bg-cyan-950/30 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6"><Lock size={32} /></div>
            <h3 className="text-2xl font-bold mb-2 dark:text-white">Protected File</h3>
            <input type="password" value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} placeholder="Password" className="w-full bg-gray-50 dark:bg-[#030712] rounded-2xl px-6 py-4 border border-transparent focus:border-cyan-500 outline-none font-bold text-center mb-4" />
            <button onClick={handleUnlock} disabled={!unlockPassword || isProcessing} className="w-full bg-cyan-500 text-white p-4 rounded-2xl font-black uppercase text-xs">Unlock</button>
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-100 dark:border-white/5 flex items-center gap-6">
            <div className="w-16 h-20 bg-gray-50 dark:bg-[#030712] rounded-xl border border-gray-100 dark:border-zinc-800 flex items-center justify-center text-cyan-500"><FileText size={24} /></div>
            ...
            <button onClick={() => setPdfData(null)} className="p-2 text-gray-400 hover:text-cyan-500"><X size={20} /></button>
          </div>
            ...
            {!extractedText ? (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setExtractionMode('text')} className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center ${extractionMode === 'text' ? 'border-cyan-500 bg-cyan-50/50 dark:bg-cyan-950/10' : 'border-gray-100 dark:border-white/5'}`}><Zap size={20} className={extractionMode === 'text' ? 'text-cyan-500' : 'text-gray-400'} /><span className="font-black uppercase text-[10px] mt-1">Fast Scan</span></button>
                  <button 
                    onClick={() => !isOcrDisabled && setExtractionMode('ocr')} 
                    disabled={isOcrDisabled}
                    className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center ${extractionMode === 'ocr' ? 'border-cyan-500 bg-cyan-50/50 dark:bg-cyan-950/10' : 'border-gray-100 dark:border-white/5'} ${isOcrDisabled ? 'opacity-40 grayscale grayscale-mask' : ''}`}
                  >
                    <ScanSearch size={20} className={extractionMode === 'ocr' ? 'text-cyan-500' : 'text-gray-400'} />
                    <span className="font-black uppercase text-[10px] mt-1">{isOcrDisabled ? 'No OCR' : 'Deep OCR'}</span>
                  </button>
                </div>
                {isProcessing && (
                  <div className="space-y-2"><div className="w-full bg-gray-100 dark:bg-zinc-800 h-2 rounded-full overflow-hidden shadow-inner"><div className="bg-cyan-500 h-full transition-all" style={{ width: `${progress}%` }} /></div><p className="text-center text-[10px] font-black text-gray-400 uppercase tracking-widest animate-pulse px-1">Scanning Document...</p></div>
                )}
                {!isProcessing && (
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-cyan-50 dark:bg-cyan-950/10 rounded-xl border border-cyan-100 dark:border-cyan-950/20 text-center">
                      <p className="text-[10px] text-cyan-500 dark:text-cyan-300 font-bold uppercase tracking-widest">Select mode and tap Extract</p>
                    </div>
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 border border-transparent focus:border-cyan-500 outline-none font-bold text-sm dark:text-white" 
                      />
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-6">
                <textarea readOnly value={extractedText} className="w-full h-80 bg-gray-50 dark:bg-[#030712] border border-gray-100 dark:border-white/5 rounded-2xl p-4 font-mono text-[10px] resize-none outline-none focus:border-cyan-500 dark:text-gray-300 shadow-inner" />
                ...
                <button onClick={() => { setExtractedText(''); setProgress(0); setPdfData(null); }} className="w-full py-2 text-gray-400 uppercase font-black text-[10px] hover:text-cyan-500 transition-colors">Close File</button>
              </div>
            )}
          </div>
        </div>
