...
export default function SplitTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-split')
  ...
  const ActionButton = () => (
    <button 
      ...
      className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl flex items-center justify-center gap-3 shadow-lg shadow-cyan-500/20`}
    >
    ...
    >
      ...
      {!pdfData ? (
        <button 
          ...
          className={`w-full border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 dark:hover:bg-cyan-950/10 transition-all cursor-pointer group ${isLoadingMeta ? 'opacity-50 cursor-wait' : ''}`}
        >
          {isLoadingMeta ? (
            ...
            <>
              <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform">
                <Scissors size={32} />
              </div>
        ...
        <div className="max-w-md mx-auto relative z-[100]">
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-2xl text-center">
            <div className="w-16 h-16 bg-cyan-100 dark:bg-cyan-950/30 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6">
              <Lock size={32} />
            </div>
            ...
            <input 
              ...
              className="w-full bg-gray-50 dark:bg-[#030712] rounded-2xl px-6 py-4 border border-transparent focus:border-cyan-500 outline-none font-bold text-center mb-4 dark:text-white"
              autoFocus
            />
            <button 
              onClick={handleUnlock}
              disabled={!unlockPassword || isLoadingMeta}
              className="w-full bg-cyan-500 text-white p-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all active:scale-95 disabled:opacity-50"
            >
              Unlock PDF
            </button>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-100 dark:border-white/5 flex items-center gap-6 shadow-sm">
            <div className="w-12 h-16 bg-gray-50 dark:bg-[#030712] rounded-xl overflow-hidden shrink-0 border border-gray-100 dark:border-zinc-800 flex items-center justify-center text-cyan-500 shadow-inner">{pdfData.thumbnail ? <img src={pdfData.thumbnail} className="w-full h-full object-cover" /> : <Scissors size={24} />}</div>
            ...
            <button onClick={() => setPdfData(null)} className="p-2 text-gray-400 hover:text-cyan-500 transition-colors"><X size={20} /></button>
          </div>
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { const all = new Set<number>(); for(let i=1;i<=pdfData.pageCount;i++) all.add(i); setSelectedPages(all); }} className="text-[10px] font-black uppercase text-cyan-500 hover:bg-cyan-50 dark:hover:bg-cyan-950/20 px-2 py-1 rounded-lg transition-colors">Select All</button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-[60vh] overflow-y-auto p-1 scrollbar-hide">
                  {Array.from({ length: pdfData.pageCount }).map((_, i) => {
                    ...
                    return (
                      <div key={pageNum} onClick={() => togglePage(pageNum)} className={`relative group cursor-pointer aspect-[3/4] rounded-xl overflow-hidden border-2 transition-all ${isSelected ? 'border-cyan-500 shadow-md scale-[1.02]' : 'border-transparent hover:border-gray-200 dark:hover:border-zinc-800'}`}>
                        <LazyThumbnail pdfDoc={pdfData.pdfDoc} pageNum={pageNum} />
                        <div className={`absolute inset-0 flex items-center justify-center transition-opacity ${isSelected ? 'bg-cyan-500/10 opacity-100' : 'bg-black/20 opacity-0 group-hover:opacity-100'}`}>
                           <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-transform ${isSelected ? 'bg-cyan-500 text-white scale-100' : 'bg-white text-gray-400 scale-75'}`}>
                ...
                <div className="space-y-6">
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2 bg-gray-50 dark:bg-[#030712] p-1 rounded-2xl">
                        <button onClick={() => { setSplitMode('single'); clearUrls(); }} className={`py-2 px-3 rounded-xl text-[9px] font-black uppercase transition-all ${splitMode === 'single' ? 'bg-white dark:bg-zinc-800 text-cyan-500 shadow-sm' : 'text-gray-400'}`}>One Document</button>
                        <button onClick={() => { setSplitMode('individual'); clearUrls(); }} className={`py-2 px-3 rounded-xl text-[9px] font-black uppercase transition-all ${splitMode === 'individual' ? 'bg-white dark:bg-zinc-800 text-cyan-500 shadow-sm' : 'text-gray-400'}`}>Separate Files</button>
                      </div>
                    ...
                    <div>
                      ...
                      <input type="text" value={customFileName} onChange={(e) => setCustomFileName(e.target.value)} className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 border border-transparent focus:border-cyan-500 outline-none font-bold text-sm dark:text-white" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">Range Selection</label>
                      <div className="flex gap-2">
                        <input type="text" value={rangeInput} onChange={(e) => setRangeInput(e.target.value)} placeholder="e.g. 1, 3-5" className="flex-1 bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 border border-transparent focus:border-cyan-500 outline-none font-bold text-sm dark:text-white" />
                        <button onClick={() => parseRange(rangeInput)} className="px-4 bg-cyan-500 text-white rounded-xl font-black text-[10px] uppercase active:scale-95 transition-transform">Apply</button>
                      </div>
                  ...
                  <div className="pt-6 border-t border-gray-100 dark:border-white/5">
                    <div className="flex justify-between items-end mb-4 px-1">
                      ...
                      <span className="text-xl font-black text-cyan-500">{selectedPages.size} <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Pages</span></span>
                    </div>
                    {objectUrl && (
                      <SuccessState 
                  ...
                  <button onClick={() => { setPdfData(null); clearUrls(); }} className="w-full py-2 text-[10px] font-black uppercase text-gray-300 hover:text-cyan-500 transition-colors">Close File</button>
                </div>
              </div>
            </div>
          </div>
