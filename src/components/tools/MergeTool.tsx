...
function SortableItem({ id, file, onRemove, onRotate, onUnlock }: { id: string, file: PdfFile, onRemove: (id: string) => void, onRotate: (id: string) => void, onUnlock: (id: string, pass: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center gap-3 p-3 bg-white dark:bg-zinc-900 rounded-2xl border transition-all shadow-sm group touch-none relative ${isDragging ? 'border-cyan-300 dark:border-cyan-700 shadow-xl scale-[1.02] ring-4 ring-cyan-500/10' : 'border-gray-100 dark:border-zinc-800 hover:border-cyan-200 dark:hover:border-cyan-950/30'}`}>
      <div {...attributes} {...listeners} className="p-2 cursor-grab text-cyan-300 hover:text-cyan-500 dark:text-cyan-500/50 dark:hover:text-cyan-300 hover:bg-cyan-500/10 rounded-lg transition-colors active:scale-90">
      ...
      <div className="w-12 h-16 bg-gray-50 dark:bg-zinc-800 rounded-lg overflow-hidden shrink-0 border border-gray-100 dark:border-zinc-800 relative group-hover:shadow-md transition-shadow">
        {file.isLocked ? (
          <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100 dark:bg-[#030712] text-cyan-500">
            <Lock size={16} />
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="font-bold text-sm truncate text-gray-900 dark:text-white group-hover:text-cyan-500 transition-colors">{file.file.name}</p>
          {file.isLocked && <Lock size={12} className="text-cyan-500 shrink-0" />}
        </div>
        ...
        {file.isLocked ? (
          <div className="flex gap-1 mt-1">
            <input 
              ...
              className="flex-1 bg-gray-50 dark:bg-[#030712] border border-gray-100 dark:border-zinc-800 rounded-lg px-2 py-1 text-[10px] font-bold outline-none focus:border-cyan-500 text-gray-900 dark:text-white"
            />
            <button 
              onClick={handleUnlockClick}
              disabled={!localPass || isUnlocking}
              className="bg-cyan-500 text-white px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest disabled:opacity-50 hover:scale-105 active:scale-95 transition-transform"
            >
              {isUnlocking ? '...' : 'Unlock'}
            </button>
          </div>
      ...
      <div className="flex items-center gap-1">
        {!file.isLocked && (
          <button 
            onClick={() => onRotate(id)}
            className="p-2 hover:bg-gray-50 dark:hover:bg-zinc-800 rounded-full text-gray-400 hover:text-cyan-500 transition-all hover:rotate-90 active:scale-90"
            title="Rotate 90°"
          >
            <RotateCw size={18} />
          </button>
        )}
        <button onClick={() => onRemove(id)} className="p-2 hover:bg-cyan-500/10 rounded-full text-gray-400 hover:text-cyan-500 transition-all hover:scale-110 active:scale-90">
          <X size={18} />
        </button>
      </div>
    </div>
...
export default function MergeTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-merged')
  ...
  return (
    ...
    >
      ...
      >
        {isDraggingGlobal && (
          <div className="fixed inset-0 z-[100] bg-cyan-500/90 backdrop-blur-xl flex flex-col items-center justify-center text-white p-6 animate-in fade-in duration-300">
          ...
          <div className="mb-8 p-6 bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/30 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-4 animate-in slide-in-from-top duration-500 shadow-sm">
             ...
             <div className="flex gap-2 w-full md:w-auto">
                ...
                <button 
                  ...
                  className="flex-1 md:flex-none px-6 py-3 bg-white dark:bg-zinc-800 text-gray-400 hover:text-cyan-500 rounded-xl text-xs font-black uppercase tracking-widest transition-colors active:scale-95"
                >
                  Discard
                </button>
             </div>
          ...
          {files.length > 0 ? (
            <div className="space-y-4">
              <div className="flex justify-between items-center px-1">
                ...
                <button onClick={() => { setFiles([]); clearUrls(); clearWorkspace('merge'); }} className="text-[10px] font-black uppercase text-cyan-500/60 hover:text-cyan-500 transition-colors font-bold">Clear All</button>
              </div>
              ...
              <button 
                ...
                className="w-full py-4 border-2 border-dashed border-gray-200 dark:border-zinc-800 rounded-2xl text-gray-400 font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 hover:border-cyan-500 hover:text-cyan-500 transition-all"
              >
              ...
              {!objectUrl && (
                <div className="p-6 bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm">
                   ...
                   <input 
                      ...
                      className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 outline-none font-bold text-sm border border-transparent focus:border-cyan-500 transition-colors dark:text-white"
                   />
                </div>
              )}
            </div>
            ...
            >
               <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform shadow-inner">
                  <Upload size={32} />
               </div>
