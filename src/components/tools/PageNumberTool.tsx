...
export default function PageNumberTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-numbered')
    ...
    <NativeToolLayout title="Page Numbers" description="Add custom numbering to your PDF automatically." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 dark:hover:bg-cyan-950/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><Hash size={32} /></div>
              ...
              {!downloadUrl ? (
                <>
                  <div>
                    ...
                    <input type="text" value={format} onChange={(e) => setFormat(e.target.value)} className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 border border-transparent focus:border-cyan-500 outline-none font-bold text-sm dark:text-white" placeholder="Page {n} of {total}" />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(['top-left', 'top-center', 'top-right', 'bottom-left', 'bottom-center', 'bottom-right'] as Position[]).map(pos => (
                        <button key={pos} onClick={() => setPosition(pos)} className={`py-3 px-1 rounded-xl text-[8px] font-black uppercase transition-all border ${position === pos ? 'bg-cyan-500 text-white border-cyan-500 shadow-lg shadow-cyan-500/20' : 'bg-gray-50 dark:bg-[#030712] text-gray-400 border-gray-100 dark:border-zinc-800'}`}>
                          {pos.replace('-', ' ')}
                        </button>
                      ))}
                    </div>
