...
export default function RotateTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-rotated')
  ...
  const ActionButton = () => (
    <button onClick={savePDF} disabled={isProcessing} className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 shadow-lg shadow-cyan-500/20 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl`}>
    ...
    <NativeToolLayout title="Rotate PDF" description="Tap individual pages to rotate 90 degrees." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 dark:hover:bg-cyan-950/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><RotateCw size={32} /></div>
        ...
        <div className="max-w-md mx-auto relative z-[100]">
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 text-center shadow-2xl">
            ...
            <input type="password" value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} placeholder="Password" className="w-full bg-gray-50 dark:bg-[#030712] rounded-2xl px-6 py-4 border border-transparent focus:border-cyan-500 outline-none font-bold text-center mb-4 dark:text-white" />
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-100 dark:border-white/5 flex items-center gap-6 shadow-sm">
            <div className="w-12 h-16 bg-gray-50 dark:bg-[#030712] rounded-xl overflow-hidden shrink-0 border border-gray-100 dark:border-zinc-800 flex items-center justify-center text-cyan-500 shadow-inner">{pdfData.thumbnail ? <img src={pdfData.thumbnail} className="w-full h-full object-cover" /> : <RotateCw size={24} />}</div>
          ...
          <div className="bg-cyan-500/5 dark:bg-cyan-500/10 border border-cyan-500/20 rounded-2xl p-4 flex items-center gap-4 animate-pulse">
            <div className="w-10 h-10 bg-cyan-500 text-white rounded-full flex items-center justify-center shrink-0 shadow-lg shadow-cyan-500/20">
              <RotateCw size={20} />
            </div>
            <div className="flex-1">
              <h4 className="text-sm font-black text-cyan-500 uppercase tracking-tight leading-none mb-1">Visual Editor</h4>
              <p className="text-xs text-cyan-500/70 font-bold">Tap any page thumbnail below to rotate it 90° clockwise.</p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={rotateAll} className="text-[10px] font-black uppercase text-cyan-500 flex items-center gap-1 font-bold"><RotateCw size={12}/> All</button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-[60vh] overflow-y-auto p-1 scrollbar-hide">
              {Array.from({ length: pdfData.pageCount }).map((_, i) => {
                ...
                return (
                  <div key={pageNum} onClick={() => rotatePage(pageNum)} className="relative group cursor-pointer aspect-[3/4] rounded-xl overflow-hidden border-2 border-transparent hover:border-cyan-500 transition-all bg-gray-50 dark:bg-[#030712] shadow-sm">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/5 transition-colors">
                      <div className="bg-white dark:bg-zinc-800 text-cyan-500 p-2 rounded-full opacity-0 group-hover:opacity-100 shadow-xl scale-75 group-hover:scale-100 transition-all border border-gray-100 dark:border-white/5">
                        <RotateCw size={20} />
                      </div>
                    </div>
          ...
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm space-y-6">
            {!downloadUrl ? (
              <div className="space-y-6">
                <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-3">Output Filename</label><input type="text" value={customFileName} onChange={(e) => setCustomFileName(e.target.value)} className="w-full bg-gray-50 dark:bg-[#030712] rounded-xl px-4 py-3 border border-transparent focus:border-cyan-500 outline-none font-bold text-sm dark:text-white" /></div>
              </div>
            ) : (
            ...
            <button onClick={() => setPdfData(null)} className="w-full py-2 text-[10px] font-black uppercase text-gray-300 hover:text-cyan-500 transition-colors">Close File</button>
          </div>
        </div>
      )}
      <PrivacyBadge />
