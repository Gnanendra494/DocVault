...
function SortablePage({ id, pageNum, pdfDoc }: { id: string, pageNum: number, pdfDoc: any }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className={`relative group cursor-grab active:cursor-grabbing aspect-[3/4] rounded-xl overflow-hidden border-2 transition-all bg-gray-50 dark:bg-[#030712] touch-none ${isDragging ? 'border-cyan-500 shadow-2xl scale-105 opacity-90' : 'border-transparent hover:border-cyan-500 shadow-sm'}`}>
...
export default function RearrangeTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-rearranged')
  ...
  const ActionButton = () => (
    <button onClick={savePDF} disabled={isProcessing} className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl flex items-center justify-center gap-3 shadow-lg shadow-cyan-500/20`}>
    ...
    <NativeToolLayout title="Rearrange PDF" description="Drag and drop to reorder pages visually." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 dark:hover:bg-cyan-950/10 transition-all cursor-pointer group">
          <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform"><Grid size={32} /></div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-100 dark:border-white/5 flex items-center gap-6 shadow-sm">
            <div className="w-12 h-16 bg-gray-50 dark:bg-[#030712] rounded-xl overflow-hidden shrink-0 border border-gray-100 dark:border-zinc-800 flex items-center justify-center text-cyan-500 shadow-inner">{pdfData.thumbnail ? <img src={pdfData.thumbnail} className="w-full h-full object-cover" /> : <Grid size={24} />}</div>
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
            <div className="flex justify-between items-center mb-6 px-2">
              ...
              <button onClick={() => setPageOrder(Array.from({ length: pdfData.pageCount }, (_, i) => (i + 1).toString()))} className="text-[10px] font-black uppercase text-gray-400 flex items-center gap-1 hover:text-cyan-500 transition-colors font-bold"><RefreshCcw size={12}/> Reset</button>
            </div>
