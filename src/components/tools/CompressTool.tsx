    ...
    <div className="space-y-4">
      ...
      <div ref={containerRef} className="relative h-80 md:h-[400px] rounded-[2rem] overflow-hidden cursor-ew-resize select-none border border-gray-100 dark:border-white/5" onMouseMove={handleMove} onTouchMove={handleMove}>
        ...
        <div className="absolute top-0 bottom-0 w-1 bg-white shadow-xl z-10" style={{ left: `${sliderPos}%` }}>
          <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-8 h-8 bg-white dark:bg-zinc-900 rounded-full shadow-2xl border border-gray-100 dark:border-white/5 flex items-center justify-center text-cyan-500">
...
export default function CompressTool() {
  ...
  const handleDownloadBatch = async () => {
    if (objectUrl && files.length > 1) {
        ...
        await downloadFile(new Uint8Array(await blob.arrayBuffer()), 'docvault-compressed.zip', 'application/zip')
    }
  }
  ...
  const ActionButton = () => (
    <button 
      ...
      className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 shadow-lg shadow-cyan-500/20 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl`}
    >
  ...
  return (
    <NativeToolLayout title="Compress PDF" description="Reduce file size while maintaining quality. Everything stays on your device." actions={files.length > 0 && !showSuccess && <ActionButton />}>
      ...
      {files.length === 0 ? (
        ...
        >
          <div className="w-20 h-20 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform shadow-inner"><Zap size={32} /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {files.map(f => (
              <div key={f.id} className="bg-white dark:bg-zinc-900 p-4 rounded-[1.5rem] border border-gray-100 dark:border-white/5 flex items-center gap-4 relative group shadow-sm">
                <div className="w-12 h-16 bg-gray-50 dark:bg-[#030712] rounded-lg overflow-hidden shrink-0 border border-gray-100 dark:border-zinc-800">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {f.isLocked ? (
                    <div className="flex gap-1 mt-1">
                       <input type="password" placeholder="Locked..." className="flex-1 bg-gray-50 dark:bg-[#030712] text-[10px] p-1.5 rounded-lg outline-none w-full border border-gray-100 dark:border-zinc-800 focus:border-cyan-500" onKeyDown={(e) => { if(e.key === 'Enter') handleUnlock(f.id, e.currentTarget.value) }} />
                    </div>
                ...
                <button onClick={() => setFiles(prev => prev.filter(item => item.id !== f.id))} className="p-2 text-gray-300 hover:text-cyan-500 transition-colors"><X size={16} /></button>
              </div>
            ))}
            <button onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-100 dark:border-zinc-800 rounded-[1.5rem] p-4 text-gray-400 flex flex-col items-center justify-center gap-1 hover:border-cyan-500 hover:text-cyan-500 transition-all">
          ...
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button key={lvl.id} onClick={() => setQuality(lvl.id as CompressionQuality)} className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-1 ${quality === lvl.id ? 'border-cyan-500 bg-cyan-50/50 dark:bg-cyan-950/10' : 'border-gray-100 dark:border-white/5'}`}>
                  <span className={`font-black uppercase text-[9px] text-center leading-tight ${quality === lvl.id ? 'text-cyan-500' : 'text-gray-400'}`}>{lvl.label}</span>
                  <span className="text-[8px] text-gray-400 font-bold uppercase">{lvl.desc}</span>
                </button>
              ))}
            </div>
            
            <div className="mt-6 p-6 bg-gray-50 dark:bg-[#030712] rounded-2xl border border-gray-100 dark:border-white/5">
               ...
               <p className="text-xs text-gray-500 dark:text-zinc-400 leading-relaxed">
                 {quality === 'high' && (
                   <>
                     ...
                     Expected reduction: <span className="text-cyan-500 font-bold">10-30%</span>.
                   </>
                 )}
                 {quality === 'medium' && (
                   <>
                     ...
                     Expected reduction: <span className="text-cyan-500 font-bold">40-60%</span>.
                   </>
                 )}
                 {quality === 'low' && (
                   <>
                     ...
                     Expected reduction: <span className="text-cyan-500 font-bold">70-90%</span>.
                   </>
                 )}
               </p>
            </div>
            ...
            {isProcessing && (
              <div className="mt-8 space-y-3">
                <div className="w-full bg-gray-100 dark:bg-zinc-800 h-2 rounded-full overflow-hidden shadow-inner">
                   <div className="bg-cyan-500 h-full transition-all" style={{ width: `${globalProgress}%` }} />
                </div>
        ...
        <div className="space-y-6 animate-in zoom-in duration-300">
          {objectUrl && files.length > 1 && (
            <button onClick={handleDownloadBatch} className="block w-full bg-zinc-900 dark:bg-white text-white dark:text-black p-10 rounded-[2.5rem] text-center shadow-2xl transition-all group active:scale-[0.98]">
              <div className="w-16 h-16 bg-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform shadow-lg"><Download className="text-white" size={32} /></div>
