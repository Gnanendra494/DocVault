...
export default function SignatureTool() {
  ...
  const [customFileName, setCustomFileName] = useState('docvault-signed')
  ...
  const ActionButton = () => (
    <button onClick={saveSignedPdf} disabled={isProcessing || !signatureImg} className={`w-full bg-cyan-500 hover:bg-cyan-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 shadow-xl shadow-cyan-500/20 ${isNative ? 'py-4 rounded-2xl text-sm' : 'p-6 rounded-3xl text-xl'}`}>
    ...
    <NativeToolLayout title="Signature" description="Sign any PDF by dragging your signature image." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <button 
          ...
          className="w-full border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-cyan-50 transition-all cursor-pointer group"
        >
          <ImageIcon size={32} className="mx-auto mb-4 text-cyan-500" />
          <h3 className="text-xl font-bold dark:text-white">Select PDF</h3>
        </button>
      ) : pdfData.isLocked ? (
        <div className="max-w-md mx-auto p-8 bg-white dark:bg-zinc-900 rounded-3xl text-center"><Lock size={32} className="mx-auto mb-4 text-cyan-500" /><input type="password" value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} className="w-full p-4 mb-4 border rounded-xl" /><button onClick={handleUnlock} className="w-full p-4 bg-cyan-500 text-white rounded-xl">Unlock</button></div>
      ) : (
        <div className="space-y-6" onMouseMove={handleMouseMove} onTouchMove={handleMouseMove} onMouseUp={() => { setIsDraggingSig(false); setIsResizing(false); }} onTouchEnd={() => { setIsDraggingSig(false); setIsResizing(false); }}>
          {!downloadUrl ? (
            <>
              <div className="bg-white dark:bg-zinc-900 p-4 rounded-3xl border border-gray-100 dark:border-white/5 relative aspect-[1/1.4] overflow-hidden touch-none" ref={previewRef} onClick={(e) => { if (!signatureImg || isDraggingSig || isResizing) return; const r = e.currentTarget.getBoundingClientRect(); setPos({ x: ((e.clientX - r.left) / r.width) * 100, y: ((e.clientY - r.top) / r.height) * 100 }) }}>
                {thumbnail ? <img src={thumbnail} className="w-full h-full object-contain" /> : <div className="w-full h-full flex items-center justify-center"><Loader2 className="animate-spin text-cyan-500" /></div>}
                {signatureImg && (
                  <div onMouseDown={(e) => { e.stopPropagation(); setIsDraggingSig(true) }} onTouchStart={(e) => { e.stopPropagation(); setIsDraggingSig(true) }} style={{ left: `${pos.x}%`, top: `${pos.y}%`, width: `${size}px`, transform: 'translate(-50%, -50%)' }} className="absolute cursor-move ring-2 ring-cyan-500 rounded-sm">
                    <img src={signatureImg} className="w-full pointer-events-none" />
                    <div onMouseDown={(e) => { e.stopPropagation(); setIsResizing(true) }} onTouchStart={(e) => { e.stopPropagation(); setIsResizing(true) }} className="absolute -bottom-2 -right-2 w-6 h-6 bg-cyan-500 rounded-full border-2 border-white cursor-nwse-resize" />
                  </div>
                )}
              </div>
              <div className="flex gap-4">
                <button onClick={() => signatureInputRef.current?.click()} className="flex-1 p-4 bg-gray-50 dark:bg-zinc-800 text-gray-900 dark:text-white border border-gray-100 dark:border-white/5 rounded-2xl font-black uppercase text-xs hover:border-cyan-500 transition-all">
          ...
          <button onClick={() => { setPdfData(null); setSignatureImg(null); }} className="w-full py-2 text-[10px] font-black uppercase text-gray-300 hover:text-cyan-500 transition-colors">Close File</button>
        </div>
      )}
      <PrivacyBadge />
    </NativeToolLayout>
