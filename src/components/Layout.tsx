...
const categoryColors: Record<ToolCategory, { bg: string, text: string, hover: string, iconBg: string }> = {
  Edit: { bg: 'bg-cyan-50 dark:bg-cyan-950/20', text: 'text-cyan-500', hover: 'hover:bg-cyan-50 dark:hover:bg-cyan-950/10', iconBg: 'bg-cyan-100 dark:bg-cyan-950/30' },
...
export default function Layout({ children, theme, toggleTheme, tools, onFileDrop, viewMode }: LayoutProps) {
  ...
  return (
    <div className={`min-h-screen flex flex-col bg-[#F5FBFF] dark:bg-[#030712] text-gray-900 dark:text-zinc-100 transition-colors duration-300`}>
      
      {isDragging && (
        <div className="fixed inset-0 z-[200] bg-cyan-500/10 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="bg-white dark:bg-zinc-900 p-12 rounded-[3rem] shadow-2xl border-4 border-dashed border-cyan-500 animate-in zoom-in duration-300">
            <UploadIcon size={64} className="text-cyan-500 animate-bounce" />
            <p className="mt-4 font-black uppercase tracking-widest text-cyan-500 text-center text-sm">Drop PDF to start</p>
          </div>
        </div>
      )}
      ...
      {!showMobileNav && (
        <header className="flex items-center justify-between px-4 md:px-8 h-16 md:h-20 border-b border-gray-100 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md sticky top-0 z-[100]">
          <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
            {!isHome && (
              <button onClick={() => navigate('/')} className="p-2 hover:bg-gray-100 dark:hover:bg-zinc-900 rounded-xl transition-colors text-gray-500 hover:text-cyan-500 shrink-0"><ArrowLeftIcon size={20} /></button>
            )}
            <Link to="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity shrink-0">
              <DocVaultLogo size={Capacitor.isNativePlatform() ? 24 : 28} iconColor="#00F2FE" />
              <span className="font-black tracking-tighter text-lg md:text-xl dark:text-white hidden xs:block">DocVault</span>
            </Link>
            <div className="h-6 w-[1px] bg-gray-200 dark:bg-zinc-800 mx-1 md:mx-2 shrink-0" />
            <div className="relative min-w-0" ref={dropdownRef}>
              <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={`flex items-center gap-2 px-2 md:px-3 py-1.5 rounded-xl transition-all font-black text-[10px] md:text-sm uppercase tracking-widest min-w-0 ${isDropdownOpen ? 'bg-cyan-500 text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-900'}`}>
          ...
          <div className="flex items-center gap-1 md:gap-3 shrink-0">
            {isMobileBrowser && (
              <a 
                href="https://github.com/yourusername/DocVault/releases/latest" 
                target="_blank"
            ...
            <Link to="/about" className={`p-2 md:px-4 md:py-2 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${location.pathname.includes('about') ? 'bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-900'}`}>
              <InfoIcon size={18} />
              <span className="hidden sm:block">About</span>
            </Link>
            <button onClick={toggleTheme} className="p-2 text-gray-400 hover:text-cyan-500 transition-colors">
              {theme === 'light' ? <MoonIcon size={20} /> : <SunIcon size={20} />}
            </button>
            <button onClick={() => setShowHistory(true)} className={`p-2 transition-colors relative ${showHistory ? 'text-cyan-500' : 'text-gray-400 hover:text-cyan-500'}`}>
              <HistoryIcon size={20} />
              {activity.length > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-cyan-500 rounded-full border-2 border-white dark:border-[#030712]" />}
            </button>
          </div>
        </header>
      )}
      ...
      {!showMobileNav && (
        <footer className="border-t border-gray-100 dark:border-white/5 mt-20 bg-white dark:bg-[#030712] relative z-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-12 gap-8 mb-12">
              ...
              <div className="col-span-2 md:col-span-6 space-y-4">
                <Link to="/" className="flex items-center gap-2.5 text-gray-900 dark:text-white group w-fit">
                  <DocVaultLogo size={22} iconColor="#00F2FE" partColor="currentColor" />
                  <span className="font-bold tracking-tight text-lg group-hover:text-cyan-500 transition-colors">DocVault</span>
                </Link>
      ...
      {shouldShowNav && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#030712] border-t border-gray-100 dark:border-zinc-800 flex items-end justify-between px-6 pb-[calc(env(safe-area-inset-bottom)+16px)] pt-3 z-[100] shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
          <button 
            onClick={() => navigate('/')}
            className={`flex flex-col items-center gap-1.5 flex-1 transition-all ${location.pathname === '/' ? 'text-cyan-500' : 'text-gray-400 dark:text-zinc-600'}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1.5 flex-1 transition-all ${location.pathname === '/android-tools' ? 'text-cyan-500' : 'text-gray-400 dark:text-zinc-600'}`}
          >
          ...
          <div className="relative -top-8">
             <button 
               ...
               className="w-14 h-14 bg-cyan-500 text-white rounded-2xl shadow-xl shadow-cyan-500/40 flex items-center justify-center active:scale-90 transition-transform ring-4 ring-white dark:ring-[#030712]"
             >
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1.5 flex-1 transition-all ${location.pathname === '/android-history' ? 'text-cyan-500' : 'text-gray-400 dark:text-zinc-600'}`}
          >
          ...
          <Link 
            to="/settings"
            className={`flex flex-col items-center gap-1.5 flex-1 transition-all no-underline ${location.pathname.includes('settings') ? 'text-cyan-500' : 'text-gray-400 dark:text-zinc-600'}`}
          >
        ...
        <div className="p-6 h-full flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <HistoryIcon className="text-cyan-500" size={24} />
            ...
            <div className="flex items-center gap-2">
              {activity.length > 0 && (
                <button 
                  ...
                  className="p-2 hover:bg-cyan-50 dark:hover:bg-cyan-950/20 text-gray-400 hover:text-cyan-500 rounded-xl transition-colors"
                >
                  <Trash2Icon size={18} />
                </button>
              )}
            ...
            {activity.length === 0 ? (<div className="text-center py-20 opacity-40"><p className="text-xs font-bold uppercase tracking_widest text-gray-400">No recent files</p></div>) : (
              activity.map((item) => (
                <div key={item.id} className="p-4 bg-gray-50 dark:bg-zinc-900/50 rounded-2xl border border-gray-100 dark:border-zinc-800 group relative">
                  <div className="flex items-center gap-3 mb-2"><div className="w-8 h-8 bg-cyan-50 dark:bg-cyan-950/20 text-cyan-500 rounded-lg flex items-center justify-center"><CheckCircleIcon size={16} /></div><div className="flex-1 min-w-0"><p className="text-xs font-bold truncate dark:text-white">{item.name}</p><p className="text-[10px] text-gray-400 font-black uppercase tracking-tighter">{item.tool}</p></div></div>
                  <div className="flex items-center justify-between text-[9px] text-gray-400 font-bold"><span>{new Date(item.timestamp).toLocaleTimeString()}</span>{item.resultUrl && (<a href={item.resultUrl} download={item.name} className="text-cyan-500 hover:underline flex items-center gap-1"><DownloadIcon size={10} /> Redownload</a>)}</div>
                </div>
              ))
            )}
          </div>
      ...
      {showHistory && (<div onClick={() => setShowHistory(false)} className="fixed inset-0 bg-black/20 dark:bg-[#030712]/60 backdrop-blur-sm z-[140] animate-in fade-in duration-300" />)}
    </div>
  )
}